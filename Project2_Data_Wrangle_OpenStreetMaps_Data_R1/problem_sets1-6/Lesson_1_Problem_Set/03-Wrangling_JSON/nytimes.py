...
def article_overview(kind, period):
    ...
    for index in range(len(data)):
        temp_title = {}
        temp_title[data[index]["section"]] = data[index]["title"]
        titles.append(temp_title)
        media_data = data[index]["media"]
        for i in range(len(media_data)):
            format_data = media_data[i]["media-metadata"]
            for num in range(len(format_data)):
                if format_data[num]["format"] == "Standard Thumbnail":
                    urls.append(format_data[num]["url"])
    # YOUR CODE HERE
