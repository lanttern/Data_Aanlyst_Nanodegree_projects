...
p1 + geom_histogram(aes(fill = status), binwidth = 0.2) + 
...
### First, total fundings vs year by market of hotstartups is explored.
...
### Then, total fundings vs state by hotstartups is explored

```{r total fundings vs state by hotstartups}
# draw line plot
...
### Next, compare total fundings vs status by hotstartups

```{r total fundings vs status by hotstartups}
# draw line plot
...
### Then, explore total fundings vs funding rounds by hotstartups
...
# draw a scatter plot to reflect number of startup in each status by market
ggplot(aes(x = status, y = freq), data = number_of_startups_by_status) + 
  scale_y_log10() +
  geom_point(aes(color = market), size =4)
```

### Observations and Interpretations for Exploration of Three or more Variables.

##### 1. Total fundings vs year by hotstartups: there are no obvious difference of getting total fundings among founded years in different markets.
##### 2. Total fundings vs state by hotstartups: in most states, health care (18 states) and biotechnology (10 states) got more total fundings
##### 3. Total fundings vs status by hotstartups: startups in health care market got more fundings when they are in acquired or operating status. While, Startups in biotechnology market got more fundings when they are in closed status.  
##### 4. Total fundings vs funding rounds by hotstartups: usually, startups got more rounds of fundings have more total fundings. 
...
p1 + geom_histogram(aes(fill = status), binwidth = 0.2) + 
  ...
  ggtitle("Histogram of total fundings for startups ($USD, log10)") + 
  scale_x_log10(breaks = c(1e+04, 1e+06, 1e+08)) +
  geom_freqpoly(binwidth = 0.5, position = "identity", aes(color = status)) + 
  facet_wrap(~ status)
```
...
p10 <- ggplot(aes(x = market, y = funding_total_usd), data = hot_startups) + scale_y_log10() + 
  ...
  ggtitle("Total fundings for each market of hot startups ($USD, log10)") + 
...
```{r Plot_Three_total fundings vs state by hotstartups}
...
```{r Plot_Three_number of startups in each status}
# count number of startups in each status by each market
number_of_startups_by_status <- count(subset(hot_startups, status  != ""), c("market","status"))
# draw a scatter plot to reflect number of startup in each status by market
ggplot(aes(x = status, y = freq), data = number_of_startups_by_status) + 
  scale_y_log10() + 
  theme(axis.text=element_text(size=10)) +
  ggtitle("Fundings ($USD, log10) for each market of hot startups in each status") + 
  labs(x = "Status", y = "Total fundings ($USD)") +
  geom_point(aes(color = market), size =4)
```

### 3.4.1 Description Four
##### There are greatest number of startups in software market in acquried, operating and closed status, suggesting fast change in this market. The biotechnology market has close number of startups to software market in operating status, but lower number of startups in closed status, which may implicate a strong rising in this market.
